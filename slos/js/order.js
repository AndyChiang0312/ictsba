...
for(let i = 1; i <= daysInMonth; i++) {
    ...
    if(day == 6 || day == 0) {
        ...
        if(meal[mealNumber] == '南瓜蘑菇燴雞球糙米飯' || meal[mealNumber] == '雙菇炆眉豆飯') {
            var A = '<td class="possibleAllergy" id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-A>' + meal[mealNumber] + '</td>';
        } else {
            var A = '<td id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-A>' + meal[mealNumber] + '</td>';
        }

        if(meal[mealNumber + 1] == '南瓜蘑菇燴雞球糙米飯' || meal[mealNumber + 1] == '雙菇炆眉豆飯') {
            var B = '<td class="possibleAllergy" id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-B> ' + meal[mealNumber + 1] + '</td>'
        } else {
            var B = '<td id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-B> ' + meal[mealNumber + 1] + '</td>'
        }

        if(meal[mealNumber + 2] == '南瓜蘑菇燴雞球糙米飯' || meal[mealNumber + 2] == '雙菇炆眉豆飯') {
            var C = '<td class="possibleAllergy" id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-C> ' + meal[mealNumber + 2] + '</td>'
        } else {
            var C = '<td id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-C> ' + meal[mealNumber + 2] + '</td>'
        }

        if(meal[mealNumber + 3] == '南瓜蘑菇燴雞球糙米飯' || meal[mealNumber + 3] == '雙菇炆眉豆飯') {
            var D = '<td class="possibleAllergy" id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-D> ' + meal[mealNumber + 3] + '</td>'
        } else {
            var D = '<td id=' + String(year) + '-' + String(month) +  '-' + String(i) + '-D> ' + meal[mealNumber + 3] + '</td>'
        }
        
        table += '<tr><td class="orderDate">' + month + '月' + String(i) + '日(' + chineseWeekday[day - 1] + ')</td>' + A + B + C + D + '<td><select id= "' + String(year) + '-' + String(month) +  '-' + String(i) + '-select" onchange="showSelect(' + String(year) + ',' + String(month) + ',' + String(i) + ');"><option value="-">-</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select></td></tr>';
