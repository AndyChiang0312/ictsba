...
for(let i = 1; i <= daysInMonth; i++) {
    ...
    if(day >= 1 && day <= 5) {
        ...
        table += '<tr><td class="orderDate">' + monthVerify(month) + '月' + String(i) + '日(' + chineseWeekday[day - 1] + ')</td>' + A + B + C + D + '<td><select id= "' + String(year) + '-' + String(monthVerify(month)) +  '-' + String(i) + '-select" onchange="showSelect(' + String(year) + ',' + String(month) + ',' + String(i) + ');" required><option value="">-</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select></td></tr>';
